...
static int session_actived_frame(unsigned char *data)
{
	...
	if (iph->protocol != IPPROTO_UDP){
		if (iph->protocol == IPPROTO_TCP){
			...
			if (tcph->dest == __constant_htons(8245)){
				DOD_DEBUG("Sending a packet to dynupdate.no-ip.com \n");
				return 0;
			}
		}
		return 1;
	...
	if (udph->dest == __constant_htons(53) && ntohs(udph->len) > (sizeof(struct udphdr) + 13 + 5)) {
		...
		*/
		...
		if (strncmp(dns, "ipcast", 6) == 0) {
			DOD_DEBUG("Sending a ipcast1.dynupdate.no-ip.com DNS query.\n");
			return 0;
		}
		if (strncmp(dns, "netgear",7) == 0) {
			DOD_DEBUG("Sending a netgear.api.oemdns.com DNS query.\n");
			return 0;
		}
		if (strncmp(dns, "dynupdate",9) == 0) {
			DOD_DEBUG("Sending a dynupdate.no-ip.com  DNS query.\n");
			return 0;
		}
		if (strncmp(dns, "members",7) == 0) {
			DOD_DEBUG("Sending a members.dyndns.org DNS query.\n");
			return 0;
		}
	}
	else if (udph->dest ==__constant_htons(8253)){
		DOD_DEBUG("Sending a packet to ipcast1.dynupdate.no-ip.com\n");
		return 0;
	}
	...
	if (proto < 0x8000) {
		...
		*skb_push(skb, 2) = 1;
		...
		if (!(ppp->active_filter &&
		      sk_run_filter(skb, ppp->active_filter) == 0) && actived == 1)
